...
try:
    ...
    target_table = "functions" if "functions" in table_names else table_names[0] if table_names else None
    ...
    if target_table:
        ...
        if res:
            ...
            res = conn.execute(f"SELECT name, project FROM {target_table} ORDER BY created_at DESC LIMIT 5").fetchall()
            print(res)
    else:
