...
async def main():
    ...
    for d in target_dirs:
        ...
        if not os.path.exists(abs_d): continue
                ...
                if file.endswith(".py") and file != "__init__.py":
                    ...
                    try:
                        ...
                        for f_data in funcs:
                            ...
                            existing = await sqlite_storage.get_function_by_name(name, SYSTEM_ORG_ID)
                            ...
                            if not emb:
                                logger.warning(f"     [Skip] Failed to generate embedding for {name}")
                                continue
                            ...
                            success = await sqlite_storage.upsert_function(final_data, SYSTEM_ORG_ID)
                            if success:
    ...
    logger.info(f"Finished! Processed: {total_processed}, Updated: {total_updated}, Skipped: {total_skipped}")
