...
async def verify_1_5():
    ...
    path_a = os.path.join("exports", "projects", "proj-a", "functions", "python", "isolated_func.py")
    path_b = os.path.join("exports", "projects", "proj-b", "functions", "python", "isolated_func.py")
    ...
    db = await get_db_connection()
    async with db.execute("SELECT project FROM logichive_function_history WHERE name='isolated_func'") as cursor:
