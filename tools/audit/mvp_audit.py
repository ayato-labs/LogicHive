...
async def run_audit():
    ...
    if success_dup:
        ...
        if saved_after_dup["version"] == 1:
            print("✅ PASSED: Deduplication worked (Version stayed at 1)")
        else:
            print(
                f"❌ FAILED: Version incremented despite identical code hash ({saved_after_dup['version']})"
            )
    else:
    ...
    if success_v2:
        ...
        if saved_v2["version"] == 2:
            ...
            print(
                f"❌ FAILED: Version did not increment despite code change ({saved_v2['version']})"
            )
    else:
    ...
    if found:
        ...
        print(
            f"✅ PASSED: Semantic search found the asset (Similarity: {match.get('similarity', 'N/A')})"
        )
    else:
        print(
            f"❌ FAILED: Semantic search could not find the relevant asset among {len(results)} results"
        )
        for r in results:
