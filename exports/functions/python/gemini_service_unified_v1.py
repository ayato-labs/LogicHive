...
class GeminiService:
    ...
    async def _call_api_async(self, model_names, prompt, is_json_output=False, response_schema=None):
        ...
        for model_name in model_names:
            try:
                ...
                if response_schema and is_gemini_model:
                    ...
                    config["response_mime_type"] = "application/json"
                response = await self.client.aio.models.generate_content(model=model_name, contents=prompt, config=config)
                if response.text:
    ...
    def _parse_json_response(self, text, prefix):
        try: return json.loads(text)
            ...
            cleaned = text.strip()
            match = re.search(r"```json\s*([\s\S]+?)\s*```", cleaned) or re.search(r"```\s*([\s\S]+?)\s*```", cleaned)
