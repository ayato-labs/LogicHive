...
def get_config_paths():
    ...
    if system == "Windows":
        ...
        paths.update({
            "Cursor": lib_app_support / "Cursor" / "User" / "globalStorage" / "heavy.cursor.cursor" / "mcp_config.json",
            "CloudCode": lib_app_support / "Code" / "User" / "globalStorage" / "googlecloudtools.cloudcode" / "mcp_config.json",
            "VSCode_Generic": lib_app_support / "Code" / "User" / "globalStorage" / "mcp_config.json",
        })
    else: # Linux
        config_dir = home / ".config"
        paths.update({
            "Cursor": config_dir / "Cursor" / "User" / "globalStorage" / "heavy.cursor.cursor" / "mcp_config.json",
            "CloudCode": config_dir / "Code" / "User" / "globalStorage" / "googlecloudtools.cloudcode" / "mcp_config.json",
            "VSCode_Generic": config_dir / "Code" / "User" / "globalStorage" / "mcp_config.json",
        })
...
def register():
    ...
    server_config = {
        "command": "uv",
        "args": ["run", "--project", root_str, "--no-sync", "python", f"{root_str}/src/mcp_server.py"],
    ...
    for name, path in paths.items():
        ...
        if "mcpServers" not in content: content["mcpServers"] = {}
        ...
        except Exception as e: print(f"[ERROR] Failed to write to {path}: {e}")
