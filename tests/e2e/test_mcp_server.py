...
async def test_mcp_tools_e2e():
    ...
    result = await mcp_server.save_function(
        name=name, code=code, description="E2E test function", dependencies=["none"]
    )
    ...
    assert "dependencies" in get_result.lower(), (
        "Markdown response must describe dependencies"
    )
...
async def test_mcp_quality_gate_rejection():
    ...
    result = await mcp_server.save_function(name=name, code=code)

    assert "Quality Gate REJECTED" in result
    # The user mandated that the error reason must be copy-pasteable for the client AI
    assert (
        "syntax" in result.lower()
        or "error" in result.lower()
        or "invalid" in result.lower()
    ), "Rejection message must contain the reason for the client AI to self-repair"
