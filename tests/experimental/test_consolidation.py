...
async def test_evaluate_quality_real_gemini():
    ...
    assert result["score"] <= 10, (
        f"Expected near 0 for syntax error, got {result['score']}"
    )
    assert (
        "syntax" in result["reason"].lower() or "colon" in result["reason"].lower()
    ), f"Expected clear syntax error reason, got: {result['reason']}"
...
"""
    ...
    assert result["score"] >= 85, (
        f"Expected 85+ for perfect code, got {result['score']}"
    )
    ...
    assert not any(indicator in reason_lower for indicator in negative_indicators), (
        f"Found negative indicator in reason: {result['reason']}"
    )
