...
async def test_rag_project_isolation(test_db):
    ...
    await do_save_async(
        name="iso_func", code=code_a, project="alpha", test_code="assert test() == 'Project A'"
    )
    await do_save_async(
        name="iso_func", code=code_b, project="beta", test_code="assert test() == 'Project B'"
    )
    ...
    for res in results_alpha:
        print(
            f"ISO_DEBUG Alpha Result: Name={res['name']}, Project={res['project']}, Code={res.get('code', 'N/A')}"
        )
    ...
    for res in results_beta:
        print(
            f"ISO_DEBUG Beta Result: Name={res['name']}, Project={res['project']}, Code={res.get('code', 'N/A')}"
        )
