...
class SystemFingerprint:
    ...
    def compare(stored: dict, current: dict) -> list[str]:
        ...
        if stored_py != current_py:
            diffs.append(f"Python Version Drift: Stored={stored.get('python_version')}, Current={current.get('python_version')}")
            
        # 3. High: CPU Arch change
        if stored.get("cpu_arch") != current.get("cpu_arch"):
            diffs.append(f"Architecture Drift: Stored={stored.get('cpu_arch')}, Current={current.get('cpu_arch')}")
        ...
        if stored.get("execution_driver") != current.get("execution_driver"):
            diffs.append(f"Execution Driver Change: Stored={stored.get('execution_driver')}, Current={current.get('execution_driver')}")
