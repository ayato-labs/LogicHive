...
class MetricsGateEvaluator(BaseEvaluator):
    ...
    async def evaluate(self, code: str, language: str, **kwargs) -> EvaluationResult:
        ...
        try:
            ...
            return EvaluationResult(
                score=0.0, reason=f"Syntax error prevented metrics analysis: {e}"
            )

        if not metrics:
            return EvaluationResult(
                score=100.0, reason="No measurable code structures (functions/classes) found."
            )
        ...
        return EvaluationResult(score=score, reason=reason, details={"metrics": metrics})
